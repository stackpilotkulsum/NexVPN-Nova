...
function GeoSpoofVisualizer({ selectedServer, isConnected }) {
    ...
    const fetchLocation = useCallback(async () => {
        ...
        try {
            ...
            if (!isNaN(lat) && !isNaN(lon)) {
                setRealLocation({ ...d, latitude: lat, longitude: lon, accuracy: 'ip' });
                setLocationError(false);
            } else {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            // Deep space background -> Light Sand background
            ...
            ctx.fillStyle = 'rgba(61, 59, 56, 0.1)';
            ...
            // Grid lines
            ctx.strokeStyle = 'rgba(61, 59, 56, 0.05)';
            ctx.lineWidth = 0.5;
            ...
            ctx.fillStyle = 'rgba(224, 220, 211, 0.8)';
            ...
            ctx.strokeStyle = 'rgba(156, 152, 145, 0.3)';
            ctx.lineWidth = 1;
            CONTINENTS.forEach(({ lat, lon }) => {
                const { x, y } = latLonToXY(lat, lon, W, H);
                ctx.beginPath();
            ...
            Object.entries(SERVER_COORDS).forEach(([id, coords]) => {
                ...
                if (isSelected) {
                    ...
                    ctx.fillStyle = `rgba(93,162,180,${0.5 + pulse * 0.3})`;
                    ctx.shadowColor = '#5da2b4';
                    ctx.shadowBlur = 5;
                    ctx.fill();
                    ctx.shadowBlur = 0;
                }
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px' }}>
                <div style={{ background:'var(--bg-elevated)', border:'1px solid rgba(255,51,102,0.3)', borderRadius:'10px', padding:'16px' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                        <div style={{ fontSize:'10px', letterSpacing:'1.5px', color:'var(--text-muted)', fontFamily:'JetBrains Mono', textTransform:'uppercase' }}>
                            🔴 Your Real Identity
                        </div>
                        {realLocation && (
                            <span style={{ fontSize:'9px', fontFamily:'JetBrains Mono', padding:'2px 6px', borderRadius:'4px', background: realLocation.accuracy === 'gps' ? 'rgba(0,255,136,0.1)' : 'rgba(255,170,0,0.1)', color: realLocation.accuracy === 'gps' ? 'var(--success)' : 'var(--warning)' }}>
                                {realLocation.accuracy === 'gps' ? '📍 GPS/Wi-Fi' : '🌐 Approximate IP'}
                            </span>
                        )}
                    </div>
                    {loading ? (
