...
function SecurityFeatures({ token, onSettingsChange }) {
 ...
 const handleDNS = (d) => { setDnsProtection(d.enabled); setLoading(null); showToast(d.enabled ? ' DNS Protection enabled' : ' DNS Protection disabled'); };
 ...
 <Card id="dns" icon="" title="DNS Leak Protection" desc="Routes all DNS through VPN tunnel" enabled={dnsProtection}>
 <div style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: '1.6' }}>
